...
export default function AdvertiserSignupPage() {
  ...
  useEffect(() => {
    ...
    if (draftRaw) {
      try {
        const draft = JSON.parse(draftRaw) as Partial<AdvertiserSignupValues>;
        Object.entries(draft).forEach(([key, value]) => {
          setValue(key as keyof AdvertiserSignupValues, value as never);
        });
        restored = true;
      } catch {}
    ...
    if (restored) {
      router.replace('/advertiser/signup');
    }
  ...
  function goToTerms() {
    ...
    router.push(`/terms?returnTo=${encodeURIComponent('/advertiser/signup')}`);
  }
