...
//     // Save everything needed for the callback
...
export const TiktokSignInButton = forwardRef<SocialSignInHandle, Props>(function TiktokSignInButton(
  { role, acceptedTerms, acceptedPromotions, onRequireTerms },
  ref,
) {
  async function handleClick() {
    if (!acceptedTerms) {
    ...
    localStorage.setItem(
      TIKTOK_PENDING_KEY,
      JSON.stringify({ role, codeVerifier, redirectUri, acceptedTerms, acceptedPromotions }),
    );
